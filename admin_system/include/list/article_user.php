...
$btn='<div class="btnsear"><span class="txt">关键字：</span><input type="hidden" value="'.$tcz['bcat'].'" id="sear_bcat"><input class="inp" type="text" id="sear_keyword" value="'.$keyword.'" onfocus="this.select()"><input class="btn" type="button" value="搜 索" onclick="search({log:\'article_user\'})"><input class="btn" type="button" value="高 级" onclick="search_more({log:\'article_user\',bcat:'.$tcz['bcat'].'})"></div>
<div class="btnright">'.goif(ispower($admin_group,'art_edit'),'<input type="button" value="发布信息" class="btn1" onclick="openshow({log:\'article_user\',bcat:'.$tcz['bcat'].'})">').goif(ispower($admin_group,'art_del'),'<input type="button" value="删除所选" class="btn1" onclick="deldata({log:\'article_user\',url:\'bcat='.$tcz['bcat'].'\'})">').'</div>';
$res.='<table class="ui_tablist" cellpadding="12" cellspacing="1">
<tr class="tr1">
<td style="width:60px">ID</td>
<td style="width:50px">排序</td>
<td>标题/名称</td>
<td style="width:160px">栏目分类</td>
<td style="width:150px">发布时间</td>
<td style="width:50px">状态</td>
<td style="width:60px">评论</td>
<td style="width:50px">外链</td>
...
foreach($list['list'] as $val){
	...
	$res.='<tr dataid="'.$val['id'].'" class="tr2" onmouseover="changelist(this,\'on\')" onmouseout="changelist(this,\'out\')" onmousedown="changelist(this,\'down\')" ondblclick="openshow({log:\'article_user\',bcat:'.$tcz['bcat'].',id:'.$val['id'].'})">
